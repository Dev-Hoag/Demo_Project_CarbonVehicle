...
public class ListingController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<Page<ListingResponse>>> getAllListings(Pageable pageable) {
        log.info("Fetching all listings with pagination: {}", pageable);

        Page<ListingResponse> response = listingService.getAllListings(pageable);
        var result = ApiResponse.<Page<ListingResponse>>builder()
                .statusCode(200)
                .message("Successfully fetched all listings")
                .data(response)
                .build();
