...
def process_trip_verified_event(event_data: Dict[str, Any], db: Session):
    ...
    try:
        ...
        logger.info(f"Processing TripVerified event for trip_id={event.trip_id}, user_id={event.user_id}")
        ...
        logger.info(f"Certificate created: id={certificate.id}, hash={certificate.cert_hash}")
...
def on_message_callback(ch, method, properties, body):
    ...
    try:
        ...
        if event_type == 'TripVerified':
            success = process_trip_verified_event(message.get('data', message), db)
