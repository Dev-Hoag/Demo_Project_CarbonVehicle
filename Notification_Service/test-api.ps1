...
# Test 1: Health Check
Write-Host "1️⃣  Testing service availability..." -ForegroundColor Yellow
try {
    ...
    Write-Host "❌ Service not available. Make sure it's running: npm run start:dev" -ForegroundColor Red
    exit 1
}
...
Write-Host "2️⃣  Registering test device token..." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "❌ Failed to register device: $_" -ForegroundColor Red
}

Write-Host ""

# Test 3: Send Test Notification
Write-Host "3️⃣  Sending test push notification..." -ForegroundColor Yellow
$notificationBody = @{
    userId = $testUserId
    type = "SYSTEM_ALERT"
    channel = "PUSH"
...
Write-Host "5️⃣  Checking unread count..." -ForegroundColor Yellow
try {
...
# Test 6: Mark as Read
if ($notificationId) {
    Write-Host "6️⃣  Marking notification as read..." -ForegroundColor Yellow
    try {
    ...
    Write-Host "6️⃣  Skipping mark as read (no notification ID)" -ForegroundColor Yellow
}

Write-Host ""

# Test 7: Get Preferences
Write-Host "7️⃣  Getting user preferences..." -ForegroundColor Yellow
try {
...
Write-Host "8️⃣  Updating preferences..." -ForegroundColor Yellow
$prefsBody = @{
    emailEnabled = $false
    pushEnabled = $true
    smsEnabled = $false
...
Write-Host "9️⃣  Testing internal notification (template-based)..." -ForegroundColor Yellow
$internalBody = @{
    userId = $testUserId
...
try {
    ...
    Write-Host "⚠️  Internal notification may have failed (template not found is expected): $_" -ForegroundColor Yellow
}
