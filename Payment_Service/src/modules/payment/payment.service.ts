...
export class PaymentService {
  ...
  async getPaymentHistory(userId: number, query?: any) {
    console.log('[Payment] getPaymentHistory called:', { userId, query });
    ...
    if (query?.status) {
      ...
      console.log('[Payment] Filter by status:', query.status);
    }

    // Filter by gateway
    if (query?.gateway) {
      where.gateway = query.gateway;
      console.log('[Payment] Filter by gateway:', query.gateway);
    }
    ...
    if (query?.fromDate) {
      ...
      console.log('[Payment] Filter fromDate:', query.fromDate);
    }
    ...
    if (query?.toDate) {
      ...
      console.log('[Payment] Filter toDate:', query.toDate);
    }
    ...
    console.log('[Payment] SQL:', queryBuilder.getSql());

    const [payments, total] = await queryBuilder.getManyAndCount();
    const totalPages = Math.ceil(total / limit);

    console.log('[Payment] Results:', { total, paymentCount: payments.length });
